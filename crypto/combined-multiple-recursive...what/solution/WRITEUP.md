...
This challenge involve's attacking the linearity of a CMRG (Combined Multiple Recursive Generator). You can find the original attack paper in section 5 of [this](https://eprint.iacr.org/2021/1204.pdf) paper. I'd like to note I didn't create the original and I don't want to take credit for someone else's work. The source code for this challenge was from an older CTF a fe years ago, and if you do some research you'll find they are  basically the same aside from parameters being changed slightly to make the challenge a bit easier and obviously the flag. The writeup's available are all in foreign langauges I don't understand and I'm too lazy to translate via google translate becease when LaTex is involved it's always a headache... I was able to understand what the author/s implied via the latex math  though, so that was more-so the core inspiration of the challenge. I did write the solution myself at least, I did not just copy and paste someone else's code, only for the challenge (with modifications of course) that's honest, that would completely defeat the original purpose of creating/releasing this challenge in the first place as it was a learning experience for me as well! 
